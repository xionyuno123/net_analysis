...
impl Port {
  ...
  fn disable_flow_ctrl(&self) {
    ...
    let prev_mode = {
      ...
      let ret =
        unsafe { dpdk::rte_eth_dev_flow_ctrl_get(self.id.raw(), &mut fc_conf) };
      if ret != 0 {
  ...
  fn configure_rss_reta(&self) {
    ...
    let mut reta_conf: Vec<dpdk::rte_eth_rss_reta_entry64> =
      Vec::with_capacity(capacity);

    for i in 0..capacity {
      let mut reta_entry64: dpdk::rte_eth_rss_reta_entry64 =
        unsafe { mem::zeroed() };
  ...
  fn configure(&self, promiscuous: bool, mtu: usize) -> Result<()> {
    ...
    let ret =
      unsafe { dpdk::rte_eth_dev_set_mtu(self.id.raw(), set_mtu as u16) };
    if ret < 0 {
      if ret == -95 {
