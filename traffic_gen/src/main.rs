        ...
        if let Ok(frame) = cap.next() {
            if let Some(mut mbuf) = mp.try_alloc() {
                ...
                if let Ok(eth) = EtherPacket::parse(cursor) {
                    if let Ok(ipv4) = Ipv4Packet::parse(eth.payload()) {
                        match ipv4.protocol() {
                            IpProtocol::TCP => {
                ...
                let bytes = &mut mbuf.data_mut()[0..4];
                NetworkEndian::write_u32(bytes, size as u32);
                mbufs.push(mbuf);
            } else {
...
fn send(port_id:u16,qid:u16,core:u32) {
    ...
    while RUN.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        for mbuf in batch.iter_mut() {
            ...
            let data = mbuf.data();
            let size = NetworkEndian::read_u32(data);
            unsafe {
                mbuf.trim_front(4);
                mbuf.extend(size as usize);
            }
            let cursor = CursorMut::new(mbuf.data_mut());
            let mut eth = EtherPacket::parse(cursor).unwrap();
            eth.set_dest_mac(DEST_MAC);
            let mut ipv4 = Ipv4Packet::parse(eth.payload()).unwrap();
